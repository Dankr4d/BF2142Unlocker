...
proc patchClient*(fs: FileStream, patchConfig: PatchConfig, laaPatch: bool) =
  ...
  fs.setPosition(parseHexInt("003293B0")) # Required? Game starts without this patch too (tested on linux)
  fs.write(byte(0x90))
  fs.write(byte(0x90))
  fs.setPosition(parseHexInt("003293B3")) # Required? Game starts without this patch too (tested on linux)
  fs.write(byte(0x35))
  fs.setPosition(parseHexInt("003293C1")) # Required? Game starts without this patch too (tested on linux)
  fs.write(byte(0x30))
  fs.write(byte(0xF2))
  fs.setPosition(parseHexInt("003293CF")) # Required? Game starts without this patch too (tested on linux)
  fs.write(byte(0x92))
  fs.setPosition(parseHexInt("003293D0")) # Required? Game starts without this patch too (tested on linux)
  fs.write(byte(0x94))
  fs.setPosition(parseHexInt("0045C97A")) # Required? Game starts without this patch too (tested on linux)
  fs.write(byte(0x88))
  fs.write(byte(0x1F))
  fs.write(byte(0xB5))
  fs.setPosition(parseHexInt("0045C981")) # Required, game crashes when removing
  fs.write(byte(0xC7))
  fs.write(byte(0x46))
  fs.write(byte(0x04))
  fs.setPosition(parseHexInt("0045C988")) # Required, game crashes when removing
  fs.write(byte(0xC7))
  fs.write(byte(0x06))
  fs.write(byte(0x01))
  fs.write(byte(0x00))
  ...
  fs.write(byte(0xEB))
  fs.write(byte(0x2A))
  fs.setPosition(parseHexInt("0045C9D8")) # Required? Game starts without this patch too (tested on linux)
  fs.write(byte(0x94))
  fs.write(byte(0x97))
