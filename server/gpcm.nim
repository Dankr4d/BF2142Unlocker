...
proc handleClient(client: Socket) =
  ...
  client.send(sdata)
  stdout.styledWriteLine(fgGreen, "<== ", fgYellow, "GPCM: ", resetStyle, sdata)
  stdout.flushFile()
  try:
    client.readLine(data, maxLength = 512, timeout = 1000)
  except TimeoutError:
    discard
  stdout.styledWriteLine(fgGreen, "==> ", fgYellow, "GPCM: ", resetStyle, data)
  stdout.flushFile()
  ...
  while true:
    try:
      ...
      discard
  stdout.styledWriteLine(fgBlue, "### ", fgYellow, "GPCM: ", resetStyle, "Client (", $client.getFd().int, ") disconnected!")
  stdout.flushFile()
  # client.close()
...
proc run*(ipAddress: IpAddress) =
  ...
  echo fmt"Gpcm server listening on {$ipAddress}:{$port} and waiting for clients!"
  while true:
    client = newSocket()
    address = ""
    gpcmServer.acceptAddr(client, address)
    stdout.styledWriteLine(fgBlue, "### ", fgYellow, "GPCM: ", resetStyle, "Client (", $client.getFd().int, ") connected from: ", address)
    stdout.flushFile()
